...
import { findOneByEmail, createUser, findById, findOneByGoogleId, updateUserById } from '../models/User.js';
import { findByEmail as findEmailVerification, deleteByEmail as deleteEmailVerification } from '../models/EmailVerification.js';
...
export async function register(req, res) {
  try {
    ...
    const user = await createUser({ nombre, correo: String(correo).toLowerCase().trim(), passwordHash });

    // Cleanup verification record
    try { await deleteEmailVerification(String(correo).toLowerCase().trim()); } catch (e) {}
...
export async function login(req, res) {
  try {
    ...
    const user = await findOneByEmail(String(correo).toLowerCase().trim());
    if (!user) return res.status(401).json({ message: 'Credenciales inválidas' });

    if (!user.password_hash) return res.status(401).json({ message: 'Credenciales inválidas' });
...
export async function me(req, res) {
  try {
    ...
    // Excluir password_hash del response
...
export async function changePassword(req, res) {
  try {
    ...
    const passwordHash = await bcrypt.hash(contrasena, 10);
    const updated = await updateUserById(user.id, { passwordHash });
...
export async function googleAuth(req, res) {
  try {
    ...
    if (!user) {
      ...
      // Conectar cuenta local existente con Google si no está enlazada
      const updates = {};
